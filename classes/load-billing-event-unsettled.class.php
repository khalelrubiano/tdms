...
try {
    ...
    $sql = "SELECT invoice_number, due_date 
    FROM billing
    WHERE billing_status = 'Unsettled'";
    ...
    for ($i = 0; $i < count($row); $i++) {
        ...
        $childArray = array('title' => 'Invoice #' . $row[$i][0], 'description' => "Invoice #" . $row[$i][0] . " is expected to be settled before " . $row[$i][1], 'start' => $row[$i][1], 'color' => '#ff9999', 'textColor' => '#000000');
